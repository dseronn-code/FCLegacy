...
FORMATO DE RETORNO OBRIGATÓRIO (JSON PURO):
{
  ...
  try {
    ...
    const financialScenarios = [
      `Ganha um salário astronômico de superestrela em 2026. Investiu pesado em um resort de ultraluxo na paradisíaca ilha de Bali, possui uma cobertura cinematográfica de €18M em Dubai Marina e é dono de sua própria marca de grife esportiva de alto padrão.`,
      `Seu contrato em 2026 garante patamares financeiros absurdos. Adquiriu uma imensa propriedade futurista projetada por arquitetos italianos renomados na Suíça, além de possuir coberturas em Miami Beach e dezenas de investimentos em startups de IA de futebol.`,
      `Considerado um dos jovens mais ricos do esporte mundial em 2026. Ostenta uma mansão de três andares com heliponto próprio na Sardenha, iate de luxo customizado com seu número histórico e uma holding que administra suas marcas de lifestyle de luxo.`
    ];
    const financialText = pickRandom(financialScenarios);

    // 4. Life History
    const lifeHistories = [
      `Cresceu nas quadras de areia e futebol de rua da periferia de sua cidade natal, onde driblava garotos muito maiores que ele. Descoberto em uma peneira aos 11 anos, subiu ao profissional como o fenômeno mais promissor da década e foi vendido ao futebol europeu em uma transação chocante avaliada em €105 milhões de euros.`,
      `Tratado desde o berço como uma joia rara da base, passou pelas mãos dos melhores preparadores físicos do país. Estreou aos 16 anos marcando gols fantásticos na liga principal e, após uma acirrada disputa entre cinco gigantes mundiais, assinou o contrato da sua vida para liderar o projeto do ${finalClub}.`,
      `Seu caminho foi marcado pela superação e por desacreditar os críticos. Considerado um 'late bloomer', explodiu no profissional marcando hat-tricks contra os rivais mais tradicionais e chamou atenção imediata dos olheiros da Europa, que pagaram uma multa rescisória recorde.`
    ];
    const historyText = pickRandom(lifeHistories);

    // 5. Living well (Mansions & vacations)
    const livingWellScenarios = [
      `Mora em uma mansão cinematográfica ultraprotegida cercada por lagos artificiais. Utiliza jatos executivos fretados de última geração para desfrutar de folgas de fim de semana em Saint-Tropez, Aspen e Miami, sempre acompanhado de seus parças mais próximos.`,
      `Vive cercado pela excelência máxima. Sua residência de luxo conta com cinema privativo, simulador de Fórmula 1 e quadra de futebol society oficial. Suas folgas de verão são passadas em iates navegando pela Costa Amalfitana ou em bangalôs luxuosos nas Maldivas.`,
      `Seu cotidiano em 2026 é digno de realeza. Divide-se entre uma mega mansão com campo de golfe particular e viagens a bordo de aviões particulares para curtir praias paradisíacas e festas privadas exclusivas nas ilhas gregas de Mykonos.`
    ];
    const livingText = pickRandom(livingWellScenarios);

    // 6. Family Relations
    const familyScenarios = [
      `A fama meteórica impôs desafios e um leve distanciamento físico devido às viagens intensas, mas o atleta compensa dando suporte financeiro ilimitado. Presenteou os pais com uma mansão espetacular em sua cidade natal e banca viagens de primeira classe para todos assistirem aos seus jogos importantes.`,
      `Mantém uma relação extremamente próxima e blindada contra o oportunismo da mídia. Seus familiares gerenciam diretamente sua carreira financeira e contratos de publicidade, e o jogador faz questão de passar todas as festas de fim de ano reunido com eles.`,
      `Apesar da rotina extenuante da elite europeia, a família continua sendo sua base de sustentação. O jogador financiou projetos sociais inovadores propostos por seus irmãos e garante passagens e estadias nos hotéis de luxo da Europa para ter seus familiares sempre por perto.`
    ];
    const familyText = pickRandom(familyScenarios);

    // 7. On-field Behavior
    const behaviorScenarios = [
      `Famoso pelas provocações ousadas em campo: comemorações icônicas mandando a torcida adversária calar a boca, fazendo 'caretas' para as câmeras e respondendo com dribles plásticos quando sofre faltas duras. É o terror dos zagueiros e queridinho dos holofotes.`,
      `Foco mental absoluto mesclado com estilo marrento incomparável. Encara rivais olho no olho, comemora seus gols decisivos tirando a camisa e chutando a bandeira de escanteio de forma marrenta, e costuma dar entrevistas provocativas e sinceras após grandes vitórias.`,
      `Incapaz de aceitar passivamente qualquer derrota. Comanda a equipe com intensidade agressiva, vibra muito a cada desarme e comemora gols com saltos acrobáticos e gestos teatrais que levam as arquibancadas ao delírio completo.`
    ];
    const behaviorText = pickRandom(behaviorScenarios);

    // 8. Supercars (Fortuna e Carros)
    const supercarsScenarios = [
      `Sua fortuna estimada em 2026 já ultrapassa os €280 milhões de euros. Na garagem, coleciona obras-primas como a novíssima Ferrari SF90 Stradale preta fosca, um Lamborghini Revuelto com pintura customizada e o cobiçado Porsche 911 GT3 RS.`,
      `Sua fortuna líquida em 2026 é avaliada em mais de €310 milhões de euros. Ostenta uma garagem digna de colecionador contendo um Bugatti Tourbillon personalizado, uma Ferrari Purosangue para o dia a dia e um raro Aston Martin Valour manual.`,
      `Patrimônio estimado em €350 milhões de euros em 2026. Adquiriu recentemente um hipercarro elétrico Rimac Nevera com aceleração insana de 0 a 100 km/h, além de possuir um McLaren Senna de fibra de carbono e uma Mercedes G63 blindada.`
    ];
    ...
    const expectationsScenarios = [
      `Considerado unanimemente por críticos de futebol mundial como o legítimo herdeiro da histórica camisa 10 e futuro vencedor indiscutível da Bola de Ouro nos próximos 3 anos.`,
      `Projetado para superar todos os recordes históricos de artilharia da liga principal. A imprensa esportiva internacional o define como o atacante mais completo e letal surgido neste século.`,
      `Apontado pelas lendas do futebol como o sucessor natural do trono das superestrelas mundiais. Sua mentalidade vitoriosa e frieza em finais o credenciam para reinar na elite europeia.`
    ];
    const expectationsText = pickRandom(expectationsScenarios);

    // 11. On-field performance
    const performanceScenarios = [
      `Dono de uma velocidade supersônica assustadora combinada com dribles imprevisíveis de futsal em pequenos espaços. Finaliza com os dois pés de forma cirúrgica e tem o controle de bola magnético.`,
      `Um meia-atacante moderno e genial no mano a mano. Inteligência tática absurda, visão de jogo digna de maestro e drible liso que desestabiliza qualquer linha defensiva adversária.`,
      `Atacante devastador no terço final do campo. Explosão muscular extraordinária, frieza cirúrgica cara a cara com o goleiro e habilidade fora do comum para inventar jogadas geniais do absoluto nada.`
    ];
    ...
    const relationshipsScenarios = [
      `Respeitado no vestiário pela liderança técnica indomável e garra exemplar nos treinos. Embora seja marrento e brincalhão de forma provocativa, é adorado por todos os jovens e pelo treinador da equipe.`,
      `Visto como o grande catalisador e coração do elenco. Tem espírito altamente competitivo e atritos saudáveis de alto rendimento durante os coletivos, sendo idolatrado pelas arquibancadas pelo carisma inabalável.`,
      `Considerado a superestrela intocável do clube. Líder por suas ações geniais em campo, embora prefira se isolar um pouco com seu círculo de amigos íntimos nas redes sociais fora do ambiente de treinamento.`
    ];
    const relationshipsText = pickRandom(relationshipsScenarios);

    // 15. Club satisfaction (Satisfação)
    const satisfactionScenarios = [
      `Completamente apaixonado pela atmosfera fervorosa do ${finalClub}. Recusou propostas de salários bilionários de clubes estatais do Oriente Médio com o claro e firme objetivo de conquistar a cobiçada Champions League.`,
      `Ama vestir a camisa histórica e pesada do clube. Expressa profundo respeito pela história do time, reiterando constantemente que o dinheiro não compra seu amor pelo futebol de elite europeu e pela torcida local.`,
      `Sente-se em casa e totalmente à vontade com seu papel de protagonista absoluto no ${finalClub}. Garante que seu compromisso contratual e sentimental com a torcida supera qualquer oferta financeira bilionária.`
    ];
    ...
    const fallbackPlayer = {
      ...
      perfil_completo_20_perguntas: {
        "1_personalidade": `Extremamente ${finalStyle.toLowerCase()} e confiante, com uma mentalidade de aço moldada para o topo. Possui uma presença magnética e um toque provocador saudável, sabendo perfeitamente do seu talento superior em campo.`,
        ...
        "20_biometria": `Altura: ${finalHeight}. Peso: ${finalWeight}. Condicionamento físico superlativo e explosão em alto rendimento.`
      },
      _is_fallback: true
    };
