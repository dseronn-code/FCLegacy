...
async function findUserDocRefAndData(userId: string) {
  ...
  try {
    ...
    if (!qSnap.empty) {
      ...
      return { ref: found.ref, id: found.id, data: found.data() };
    }
  } catch (_) {}

  try {
    const qEmail = query(collection(db, "users"), where("email", "==", cleaned.toLowerCase()));
...
// Helper to reliably grant/apply user plan entitlements across all user references
async function applyUserEntitlements(userId: string, type: string, userEmail?: string) {
  const nowIso = new Date().toISOString();
  const userObj = await findUserDocRefAndData(userId);
  if (userObj) {
    const targetRef = userObj.ref;
    const targetUserId = userObj.data?.userId || userObj.id || userId;
    
    let proExpiresAt = userObj.data?.proExpiresAt;
    let isPro = userObj.data?.isPro || false;
    let aiCredits = userObj.data?.aiCredits || 3;
    let extraSlots = userObj.data?.extraSlots || 0;
    let boostedUntil = userObj.data?.boostedUntil || null;
    ...
    } else if (type === "extra_slot") {
      extraSlots = (extraSlots || 0) + 1;
    } else if (type === "boost") {
    ...
    if (userObj.id !== targetUserId && targetUserId) {
      try {
        ...
        if (directSnap.exists()) {
          ...
          await setDoc(directRef, { ...userObj.data, ...updates });
        }
      } catch (_) {}
    }

    if (userId && userId !== userObj.id && userId !== targetUserId) {
      try {
        const rawRef = doc(db, "users", userId);
        await setDoc(rawRef, { ...userObj.data, ...updates }, { merge: true });
      } catch (_) {}
    }
    ...
    return { ref: newUserDocRef, id: userId, data: newUser };
  }
}
...
app.post("/api/admin/users/:userId/add-plan", async (req, res) => {
  try {
    ...
    await updateDoc(userObj.ref, updateFields);

    // Sync to direct user documents if ID differs
    if (userObj.id !== targetUserId && targetUserId) {
      try {
        const directRef = doc(db, "users", targetUserId);
        const directSnap = await getDoc(directRef);
        if (directSnap.exists()) {
          await updateDoc(directRef, updateFields);
        } else {
          await setDoc(directRef, { ...userObj.data, ...updateFields });
        }
      } catch (_) {}
    }
    if (userId && userId !== userObj.id && userId !== targetUserId) {
      try {
        const rawRef = doc(db, "users", userId);
        await setDoc(rawRef, { ...userObj.data, ...updateFields }, { merge: true });
      } catch (_) {}
    }
...
app.post("/api/admin/users/:userId/remove-plan", async (req, res) => {
  try {
    ...
    await updateDoc(userObj.ref, revokeFields);
    ...
    if (userObj.id !== targetUserId && targetUserId) {
      try {
        ...
        const directSnap = await getDoc(directRef);
        if (directSnap.exists()) {
          await updateDoc(directRef, revokeFields);
        }
      } catch (_) {}
    }
    if (userId && userId !== userObj.id && userId !== targetUserId) {
      try {
        const rawRef = doc(db, "users", userId);
        await setDoc(rawRef, revokeFields, { merge: true });
      } catch (_) {}
    }
