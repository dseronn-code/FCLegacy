...
Você deve utilizar a ferramenta Google Search integrada para buscar jogadores, clubes ou tendências do futebol real em 2026, ou inventar um jogador original com base em nacionalidades reais.
Retorne um JSON contendo:
- suggestedName: Um nome completo de jogador altamente realista baseado no país de origem (evite nomes ultra clichês, gere nomes variados, elegantes e originais).
...
Retorne APENAS o JSON cru válido, sem marcação de markdown (sem \`\`\`json):
{
  ...
  "personalityType": "..."
    ...
    // Static fallback if AI fails or no keys (ultra large dynamic database so it never repeats)
    const fallbackNames = [
    ...
    const fallbackNationalities = ["Brasil", "Portugal", "Espanha", "Argentina", "França", "Inglaterra", "Itália", "Alemanha", "Holanda", "Uruguai", "Bélgica"];
    ...
    return res.json({
      suggestedName: fallbackNames[Math.floor(Math.random() * fallbackNames.length)],
      nationality: fallbackNationalities[Math.floor(Math.random() * fallbackNationalities.length)],
