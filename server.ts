...
app.post("/api/send-verification-code", async (req, res) => {
  ...
  // Try to send email via SMTP/Resend if configured, else fallback gracefully
  const resendApiKey = process.env.RESEND_API_KEY;
  const smtpHost = resendApiKey ? 'smtp.resend.com' : process.env.SMTP_HOST;
  const smtpPort = resendApiKey ? 465 : (process.env.SMTP_PORT ? parseInt(process.env.SMTP_PORT) : 587);
  const smtpUser = resendApiKey ? 'resend' : process.env.SMTP_USER;
  const smtpPass = resendApiKey ? resendApiKey : process.env.SMTP_PASS;
  const smtpFrom = resendApiKey ? '"Suporte Wolkstore" <support@wolkstore.shop>' : (process.env.SMTP_FROM || smtpUser || "no-reply@wolkstore.shop");
  ...
  if (smtpUser && smtpPass && smtpHost) {
    try {
      ...
      const mailOptions = {
        from: smtpFrom,
        replyTo: resendApiKey ? 'support@wolkstore.shop' : undefined,
        to: email,
