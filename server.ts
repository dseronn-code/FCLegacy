...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    uSnap.forEach((uDoc) => {
      ...
      if (data.passwordHash === inputHash || data.password === String(password)) {
        authenticatedUser = {
          uid: data.userId || data.uid || uDoc.id,
          email: cleanEmail,
          displayName: data.username || cleanEmail,
  ...
  try {
    ...
    if (userDoc.exists()) {
      ...
      if (uData?.email === "dseronn@gmail.com" || uData?.isAdmin === true) {
        if (uData?.email === "dseronn@gmail.com" && !uData?.isAdmin) {
