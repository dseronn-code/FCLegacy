...
async function findUserDocRefAndData(userId: string) {
  ...
  try {
    const qEmail = query(collection(db, "users"), where("email", "==", userId.toLowerCase().trim()));
...
async function applyUserEntitlements(userId: string, type: string, userEmail?: string) {
  ...
  if (userObj) {
    ...
    if (type === "pro") {
      ...
      const currentSlots = userObj.data?.extraSlots || 0;
      await updateDoc(targetRef, {
    ...
    // Direct mirror update to user.uid document if different from found doc ID
    if (userObj.id !== userId && userId && userId !== "undefined") {
      try {
        const directRef = doc(db, "users", userId);
        const directSnap = await getDoc(directRef);
        if (directSnap.exists()) {
          if (type === "pro" || type === "teste") {
            await updateDoc(directRef, {
              isPro: true,
              aiCredits: 9999,
              proExpiresAt: new Date(Date.now() + (type === "pro" ? 30 : 1) * 24 * 60 * 60 * 1000).toISOString(),
              updatedAt: nowIso
            });
          }
        }
    ...
    // If no document exists yet, create it with user ID
