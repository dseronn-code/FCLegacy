...
app.post("/api/send-verification-code", async (req, res) => {
  ...
  // Return HTTP 200 immediately (success) with code included so user is never blocked
  return res.json({
    success: true,
    emailSent: hasConfig,
    debugMode: true,
    code: code,
    message: hasConfig 
      ? "Código enviado com sucesso! Caso não chegue na sua caixa de entrada, utilize o código de segurança exibido na tela." 
...
const handleResendCode = async (req: express.Request, res: express.Response) => {
  ...
  return res.json({
    ...
    code: code,
    message: hasConfig 
      ? "Um novo código de verificação foi enviado! Caso não chegue na sua caixa de entrada, utilize o código exibido na tela." 
...
app.post("/api/auth/forgot-password", async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      message: "Código de segurança enviado com sucesso! Se não chegar no e-mail, utilize o código exibido na tela.",
      debugMode: true,
      code: code,
      token: token
    });
...
// POST save or update a career document
app.post("/api/careers", async (req, res) => {
  try {
    const career = req.body;
    if (!career || !career.id) {
      return res.status(400).json({ error: "Dados inválidos da carreira." });
    }

    const userId = (req.headers["x-user-id"] as string) || career.userId;
    if (!userId) {
      return res.status(401).json({ error: "Não autorizado." });
    }

    // Ensure career object has userId
    const careerToSave = {
      ...career,
      userId,
      updatedAt: new Date().toISOString()
    ...
    await setDoc(doc(db, "careers", career.id), careerToSave);
    return res.json({ success: true, career: careerToSave });
  } catch (error: any) {
    console.error("Erro detalhado ao salvar carreira no backend:", error?.message || error);
    return res.status(500).json({ error: "Erro ao salvar carreira no servidor.", details: error?.message });
  }
});
...
// Helper to reliably locate a user document by Doc ID, userId field, or email
async function findUserDocRefAndData(userId: string) {
  if (!userId || userId === "undefined" || userId === "null") return null;
  const cleaned = decodeURIComponent(String(userId).trim());
  const lowerCleaned = cleaned.toLowerCase();

  try {
    const docRef = doc(db, "users", cleaned);
    const snap = await getDoc(docRef);
    if (snap.exists()) {
      return { ref: docRef, id: snap.id, data: snap.data() as any };
    }
  } catch (_) {}
  ...
  try {
    ...
    if (!qSnap.empty) {
      ...
      return { ref: found.ref, id: found.id, data: found.data() as any };
    }
  } catch (_) {}

  try {
    const qEmail = query(collection(db, "users"), where("email", "==", lowerCleaned));
    const qEmailSnap = await getDocs(qEmail);
    if (!qEmailSnap.empty) {
      const found = qEmailSnap.docs[0];
      return { ref: found.ref, id: found.id, data: found.data() as any };
    }
  } catch (_) {}
...
// Helper to reliably grant/apply user plan entitlements across all user references in Firestore
async function applyUserEntitlements(identifier: string, planType: string = "pro", userEmail?: string, customDays?: number) {
  const nowIso = new Date().toISOString();
  const userObj = await findUserDocRefAndData(identifier);

  const daysCount = customDays && Number(customDays) > 0 
    ? Number(customDays) 
    : (planType === "teste" ? 1 : (planType === "boost" ? 7 : 30));
  ...
  if (userObj) {
    extraSlots = userObj.data?.extraSlots || 0;
  }

  if (planType === "pro" || planType === "teste" || planType === "custom") {
    isPro = true;
    aiCredits = 9999;
    proExpiresAt = expirationDate;
  } else if (planType === "boost") {
    isPro = userObj?.data?.isPro || false;
    aiCredits = userObj?.data?.aiCredits || 3;
    proExpiresAt = userObj?.data?.proExpiresAt || null;
    boostedUntil = expirationDate;
  } else if (planType === "extra_slot") {
    isPro = userObj?.data?.isPro || false;
    aiCredits = userObj?.data?.aiCredits || 3;
    proExpiresAt = userObj?.data?.proExpiresAt || null;
    extraSlots += 1;
  ...
  if (boostedUntil) {
    updates.boostedUntil = boostedUntil;
  }

  if (userObj) {
    const targetUserId = userObj.data?.userId || userObj.id || identifier;

    // Update primary doc
    await updateDoc(userObj.ref, updates);

    // Sync across all alternate doc IDs
    const docsToSync = new Set<string>();
    docsToSync.add(userObj.id);
    if (userObj.data?.userId) docsToSync.add(userObj.data.userId);
    if (identifier) docsToSync.add(decodeURIComponent(identifier).trim());

    for (const dId of docsToSync) {
      if (!dId || dId === "undefined" || dId === "null") continue;
      try {
        const dRef = doc(db, "users", dId);
        const dSnap = await getDoc(dRef);
        if (dSnap.exists()) {
          await updateDoc(dRef, updates);
        } else {
          await setDoc(dRef, { ...userObj.data, ...updates }, { merge: true });
        }
      } catch (_) {}
    }

    if (userObj.data?.email) {
      try {
        const qE = query(collection(db, "users"), where("email", "==", userObj.data.email.toLowerCase().trim()));
        const qESnap = await getDocs(qE);
        qESnap.forEach(async (dSnap) => {
          if (dSnap.id !== userObj.id) {
            await updateDoc(dSnap.ref, updates);
          }
        });
      } catch (_) {}
    }
    ...
    if (planType === "boost") {
      try {
        const careersRef = collection(db, "careers");
        const cq = query(careersRef, where("userId", "==", targetUserId));
    ...
    return { ref: userObj.ref, id: userObj.id, data: { ...userObj.data, ...updates }, expirationDate };
  } else {
    // User doc doesn't exist yet, create it
    const cleanId = decodeURIComponent(identifier).trim();
    const newUserDocRef = doc(db, "users", cleanId);
    const newUser: any = {
      userId: cleanId,
      email: userEmail || "",
    ...
    return { ref: newUserDocRef, id: cleanId, data: newUser, expirationDate };
  }
}

// Helper to reliably revoke user plan entitlements across all user references in Firestore
async function revokeUserEntitlements(identifier: string) {
  const nowIso = new Date().toISOString();
  const userObj = await findUserDocRefAndData(identifier);
  ...
  // Update primary doc
  await updateDoc(userObj.ref, revokeUpdates);
  ...
  for (const dId of docsToSync) {
    ...
    try {
      ...
      const dSnap = await getDoc(dRef);
      if (dSnap.exists()) {
        await updateDoc(dRef, revokeUpdates);
      }
    } catch (_) {}
  }
  ...
  if (userObj.data?.email) {
    try {
      ...
      const qESnap = await getDocs(qE);
      qESnap.forEach(async (dSnap) => {
        await updateDoc(dSnap.ref, revokeUpdates);
      });
    } catch (_) {}
  }

  // Remove boost from careers
  try {
    const careersRef = collection(db, "careers");
    const cq = query(careersRef, where("userId", "==", targetUserId));
    const cSnap = await getDocs(cq);
    ...
    console.error("Erro ao revogar boost das carreiras:", err);
  }

  return { ref: userObj.ref, id: userObj.id, data: { ...userObj.data, ...revokeUpdates } };
}
...
async function checkAndDeductAiCredits(userId: string | undefined, simulateOnly: boolean = false): Promise<{ allowed: boolean; error?: string }> {
  ...
  try {
    const userObj = await findUserDocRefAndData(userId);
    if (userObj) {
      const udata = userObj.data;
      if (udata.isBlocked) {
        return { allowed: false, error: "Sua conta está bloqueada pelo administrador." };
      }
      
      const isProActive = udata.isPro && (!udata.proExpiresAt || new Date(udata.proExpiresAt).getTime() > Date.now());
      if (!isProActive) {
      ...
      if (!simulateOnly) {
        ...
        const newCredits = isProActive ? 9999 : Math.max(0, (udata.aiCredits === undefined ? 3 : udata.aiCredits) - 1);
        await updateDoc(userObj.ref, {
    ...
    console.error("Erro no checkAndDeductAiCredits:", err);
    return { allowed: true };
  }
}
...
app.post("/api/payments/create-checkout", async (req, res) => {
  try {
    const { userId, userEmail, type, planId } = req.body;
    if (!userId) {
    ...
    if (planId) {
      try {
        ...
        if (pSnap.exists()) {
          ...
          amount = Number(pData.price) || amount;
          description = `FC Legacy - ${pData.name}`;
          planType = pData.type || "pro";
          daysCount = Number(pData.days) || 30;
        }
      } catch (_) {}
    } else {
      if (type === "extra_slot") {
    ...
    const subRecord: any = {
      ...
      description,
      createdAt: new Date().toISOString(),
      expiresAt: new Date(Date.now() + daysCount * 24 * 60 * 60 * 1000).toISOString()
    };
...
app.post("/api/payments/webhook", async (req, res) => {
  try {
    ...
    if (paymentStatus === "approved") {
      ...
      await applyUserEntitlements(subData.userId, subData.type, subData.userEmail);

      console.log(`[PAYMENT Webhook] Pagamento aprovado com sucesso! Id: ${subscriptionId}`);
      
      const userEmail = subData.userEmail;
      if (userEmail) {
...
// Instant / Direct Simulated Approval Endpoint (used in UI for testing/demo or manual Pix confirmation)
app.post("/api/payments/simulate-approve", async (req, res) => {
  try {
    ...
    console.log(`[SIMULATE APPROVE] Transação ${subscriptionId} aprovada para o usuário ${subData.userId}`);
    ...
    return res.json({ success: true, message: "Pagamento aprovado com sucesso! Plano e benefícios ativados." });
  } catch (error: any) {
    console.error("Erro ao simular aprovação de pagamento:", error);
    return res.status(500).json({ error: "Erro interno ao aprovar pagamento." });
  }
});
...
// Verify single-use time-bound reactivation token
app.post("/api/payments/verify-reactivation", async (req, res) => {
  try {
    ...
    if (!reactData) {
      return res.status(404).json({ error: "Nenhuma reativação simplificada elegível foi encontrada para este e-mail." });
    }
...
app.post("/api/payments/execute-reactivation", async (req, res) => {
  try {
    ...
    await applyUserEntitlements(userId, "pro", email);
  ...
  try {
    ...
    if (subData.status !== "approved") {
      if (subData.mpPaymentId) {
        const mpToken = process.env.MERCADO_PAGO_ACCESS_TOKEN;
        if (mpToken && mpToken.trim() !== "") {
          console.log(`[STATUS POLL] Verificando status do pagamento ${subData.mpPaymentId} no Mercado Pago...`);
          try {
            ...
            if (mpResponse.ok) {
              ...
              if (paymentData.status === "approved") {
                console.log(`[STATUS POLL] Pagamento aprovado na API do MP! Liberando benefícios para ${subData.userId}`);
...
app.post("/api/admin/users/:userId/toggle-pro", async (req, res) => {
  try {
    ...
    const nextProState = isPro !== undefined ? !!isPro : !userObj.data.isPro;
    await updateDoc(userObj.ref, {
      isPro: nextProState,
...
app.post("/api/admin/users/:userId/reset-ai", async (req, res) => {
  try {
    ...
    await updateDoc(userObj.ref, {
      ...
      aiCredits: userObj.data.isPro ? 9999 : 3,
      updatedAt: new Date().toISOString()
    });
...
app.post("/api/admin/users/:userId/ban", async (req, res) => {
  try {
    ...
    const isBlocked = !userObj.data.isBlocked;
    await updateDoc(userObj.ref, {
      isBlocked,
      updatedAt: new Date().toISOString()
    });
...
app.post("/api/admin/users/:userId/toggle-admin", async (req, res) => {
  try {
    ...
    const isAdmin = !userObj.data.isAdmin;
    await updateDoc(userObj.ref, {
      isAdmin,
      updatedAt: new Date().toISOString()
    });
...
app.post("/api/admin/users/:userId/add-plan", async (req, res) => {
  try {
    const adminId = (req.headers["x-user-id"] as string) || "admin_master";
    ...
    const result = await applyUserEntitlements(userId, planType || "pro", undefined, Number(days) || 30);
    if (!result) {
      return res.status(404).json({ error: "Usuário não localizado no banco de dados." });
    }

    const targetUserId = result.data?.userId || result.id || userId;
    const nowIso = new Date().toISOString();
    const expirationDate = result.expirationDate || new Date(Date.now() + (Number(days) || 30) * 24 * 60 * 60 * 1000).toISOString();

    // Create a subscription record in ledger
    ...
    await setDoc(doc(db, "subscriptions", subId), {
      ...
      type: planType || "pro",
      description: `Cortesia Admin: Plano ${String(planType).toUpperCase()} (${Number(days) || 30} dias)`,
      createdAt: nowIso,
...
app.post("/api/admin/users/:userId/remove-plan", async (req, res) => {
  try {
    ...
    const { userId } = req.params;
    const result = await revokeUserEntitlements(userId);
    if (!result) {
      return res.status(404).json({ error: "Usuário não localizado no banco de dados." });
    }
    ...
    await setDoc(doc(db, "subscriptions", subId), {
      ...
      status: "cancelled",
      amount: 0,
      paymentMethod: "REVOGADO_ADMIN",
      type: "revoked",
      description: `Plano revogado pelo Administrador`,
      createdAt: nowIso,
      expiresAt: nowIso
    });
...
// ===================================================================
// SPOTIFY INTEGRATION ENDPOINTS (OAuth 2.0 PKCE / Authorization Code)
// ===================================================================

// Helper to retrieve & auto-refresh Spotify access token
async function getSpotifyAccessToken(userId: string): Promise<string | null> {
  if (!userId) return null;
  const userObj = await findUserDocRefAndData(userId);
  if (!userObj || !userObj.data?.spotifyConnected) return null;

  const { spotifyAccessToken, spotifyRefreshToken, spotifyTokenExpiresAt } = userObj.data;

  // Return access token if valid for at least 60 seconds
  if (spotifyAccessToken && spotifyTokenExpiresAt && Date.now() < (Number(spotifyTokenExpiresAt) - 60000)) {
    return spotifyAccessToken;
  }

  if (!spotifyRefreshToken) return spotifyAccessToken || null;

  const clientId = process.env.SPOTIFY_CLIENT_ID || "60840d126da24803a48d71f6da08755a";
  const clientSecret = process.env.SPOTIFY_CLIENT_SECRET || "5c379343c4c94e3da64e081770d69817";
  ...
  try {
    ...
    if (data.access_token) {
      ...
      if (data.refresh_token) {
        updates.spotifyRefreshToken = data.refresh_token;
      }
      await updateDoc(userObj.ref, updates);
...
app.get("/api/spotify/auth-url", (req, res) => {
  try {
    ...
    const redirectUri = process.env.SPOTIFY_REDIRECT_URI || "https://wolkstore.shop/api/spotify/callback";
    ...
    console.error("Erro ao gerar URL do Spotify:", err);
...
app.get(["/api/spotify/callback", "/api/spotify/callback/"], async (req, res) => {
  try {
    ...
    if (error || !code) {
      return res.send(`
        <html>
          <body style="background:#09090b;color:#f43f5e;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;">
            <div style="text-align:center;">
              <h2>Erro de Autenticação com Spotify</h2>
              <p>${error || "Código de autorização não recebido."}</p>
              <button onclick="window.close()" style="padding:10px 20px;background:#18181b;color:#fff;border:1px solid #27272a;border-radius:8px;cursor:pointer;">Fechar</button>
            </div>
          </body>
        </html>
      `);
    ...
    const userId = (state as string) || "guest_user";
    const clientId = process.env.SPOTIFY_CLIENT_ID || "60840d126da24803a48d71f6da08755a";
    const clientSecret = process.env.SPOTIFY_CLIENT_SECRET || "5c379343c4c94e3da64e081770d69817";
    ...
    if (tokenData.access_token) {
      ...
      return res.send(`
        <html>
          <body style="background:#09090b;color:#1DB954;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;">
            <div style="text-align:center;">
              <h2 style="font-size:24px;margin-bottom:10px;">🟢 Conectado ao Spotify com Sucesso!</h2>
              <p style="color:#a1a1aa;font-size:14px;">Fechando janela e sincronizando com o FC Legacy...</p>
              <script>
                if (window.opener) {
      ...
      return res.status(400).send(`Erro ao obter tokens do Spotify: ${JSON.stringify(tokenData)}`);
    }
  } catch (err: any) {
    console.error("[SPOTIFY CALLBACK ERROR]", err);
    return res.status(500).send("Erro interno durante autenticação do Spotify.");
  }
});
...
app.post("/api/spotify/disconnect", async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: "Spotify desconectado com sucesso." });
  } catch (err) {
    console.error("Erro ao desconectar Spotify:", err);
    return res.status(500).json({ error: "Erro ao desconectar Spotify." });
  }
});

// 4. Get Spotify Connection Status
...
// 5. Get Spotify Player State (Current track, progress, status, active device, queue)
app.get("/api/spotify/player/state", async (req, res) => {
  try {
    ...
    if (!playerRes.ok) {
      return res.json({ connected: true, isPlaying: false, track: null });
    }

    const playerData = await playerRes.json();

    // Fetch queue for "Next track"
    ...
    return res.json({
      ...
      device: playerData.device ? {
        id: playerData.device.id,
        name: playerData.device.name,
    ...
    console.error("[SPOTIFY STATE ERROR]", err);
    return res.status(500).json({ error: "Erro ao buscar estado do Spotify." });
  }
});

// 6. Play / Pause toggle
app.post("/api/spotify/player/play-pause", async (req, res) => {
  try {
    const { userId, action } = req.body; // action: "play" | "pause"
    const accessToken = await getSpotifyAccessToken(userId);
    if (!accessToken) return res.status(401).json({ error: "Spotify não conectado." });

    const endpoint = action === "pause" ? "pause" : "play";
    const spotifyRes = await fetch(`https://api.spotify.com/v1/me/player/${endpoint}`, {
      method: "PUT",
      headers: { "Authorization": `Bearer ${accessToken}` }
    });
    ...
    if (spotifyRes.ok || spotifyRes.status === 204) {
      ...
      return res.status(400).json({ error: errData.error?.message || "Não foi possível alternar reprodução." });
    }
  } catch (err) {
    return res.status(500).json({ error: "Erro ao controlar reprodução." });
  }
});
...
app.post("/api/spotify/player/next", async (req, res) => {
  try {
    ...
    return res.json({ success: spotifyRes.ok || spotifyRes.status === 204 });
  } catch (err) {
    return res.status(500).json({ error: "Erro ao avançar faixa." });
  }
});
...
app.post("/api/spotify/player/previous", async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Erro ao voltar faixa." });
  }
});

// 9. Seek position
app.post("/api/spotify/player/seek", async (req, res) => {
  try {
    const { userId, positionMs } = req.body;
    const accessToken = await getSpotifyAccessToken(userId);
    if (!accessToken) return res.status(401).json({ error: "Spotify não conectado." });

    const spotifyRes = await fetch(`https://api.spotify.com/v1/me/player/seek?position_ms=${Math.floor(positionMs)}`, {
      method: "PUT",
      headers: { "Authorization": `Bearer ${accessToken}` }
    });
    ...
    return res.status(500).json({ error: "Erro ao alterar posição da faixa." });
  }
});

// 10. Volume control
app.post("/api/spotify/player/volume", async (req, res) => {
  try {
    const { userId, volumePercent } = req.body;
    const accessToken = await getSpotifyAccessToken(userId);
    if (!accessToken) return res.status(401).json({ error: "Spotify não conectado." });

    const spotifyRes = await fetch(`https://api.spotify.com/v1/me/player/volume?volume_percent=${Math.min(100, Math.max(0, volumePercent))}`, {
      method: "PUT",
      headers: { "Authorization": `Bearer ${accessToken}` }
    });
    ...
    return res.status(500).json({ error: "Erro ao alterar volume." });
  }
});
...
app.get("/api/spotify/playlists", async (req, res) => {
  try {
    ...
    if (!playlistsRes.ok) return res.json([]);
...
// 12. Play specific track or playlist context
app.post("/api/spotify/player/play-track", async (req, res) => {
  try {
    const { userId, contextUri, trackUri } = req.body;
    const accessToken = await getSpotifyAccessToken(userId);
    if (!accessToken) return res.status(401).json({ error: "Spotify não conectado." });

    const bodyObj: any = {};
    if (contextUri) {
      bodyObj.context_uri = contextUri;
    } else if (trackUri) {
      bodyObj.uris = [trackUri];
    }
    ...
    return res.status(500).json({ error: "Erro ao reproduzir seleção." });
  }
});


// Delete user career (user or admin)
app.delete("/api/careers/:careerId", async (req, res) => {
  try {
    const { careerId } = req.params;
    ...
    if (!careerId) {
      return res.status(400).json({ error: "ID da carreira é obrigatório." });
    }
    ...
    if (!careerSnap.exists()) {
      return res.json({ success: true, message: "Carreira já excluída." });
    }

    const careerData = careerSnap.data();
    const isOwner = (userId && careerData.userId === userId) || 
                    (userEmail && careerData.userEmail?.toLowerCase() === userEmail?.toLowerCase());
    const adminCheck = userId ? await isAdminUser(userId) : false;

    if (!isOwner && !adminCheck) {
      return res.status(403).json({ error: "Você não tem permissão para apagar esta carreira." });
    }

    await deleteDoc(careerRef);
    return res.json({ success: true, message: "Carreira excluída com sucesso!" });
  } catch (error: any) {
    console.error("Erro ao deletar carreira via backend:", error);
    return res.status(500).json({ error: "Erro ao excluir carreira no servidor." });
  }
});
...
app.get("/api/admin/plans", async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn("Erro ao ler coleção plans do Firestore:", dbErr);
    }
