...
import { X, Settings, Sparkles, AlertCircle, Loader2, Check, CreditCard, ShieldAlert, Music, RefreshCw } from "lucide-react";
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const [spotifyConnected, setSpotifyConnected] = useState<boolean>(!!userProfile?.spotifyConnected);
  const [connectingSpotify, setConnectingSpotify] = useState<boolean>(false);
  const [disconnectingSpotify, setDisconnectingSpotify] = useState<boolean>(false);

  // Sync spotify connection status on open or profile change
  useEffect(() => {
    if (userProfile?.userId) {
      setSpotifyConnected(!!userProfile?.spotifyConnected);
      fetch(`/api/spotify/token?userId=${encodeURIComponent(userProfile.userId)}`)
        .then((res) => res.json())
        .then((data) => {
          if (data.connected !== undefined) setSpotifyConnected(data.connected);
        })
        .catch(() => {});
    }
  }, [isOpen, userProfile]);
  ...
  const handleConnectSpotify = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/spotify/auth-url?userId=${encodeURIComponent(userProfile.userId)}`);
      const data = await res.json();
      if (res.ok && data.url) {
        const popup = window.open(data.url, "spotify_oauth_settings", "width=600,height=700");

        // Poll token and window status
        let attempts = 0;
        const checkInterval = setInterval(async () => {
          attempts++;
          try {
            const tokenRes = await fetch(`/api/spotify/token?userId=${encodeURIComponent(userProfile.userId)}`);
          ...
          if (popup && popup.closed) {
            ...
            fetch(`/api/spotify/token?userId=${encodeURIComponent(userProfile.userId)}`)
              .then((r) => r.json())
              .then((d) => { if (d.connected) setSpotifyConnected(true); })
              .catch(() => {});
          }
        ...
        setError(data.error || "Erro ao obter URL do Spotify.");
        setConnectingSpotify(false);
      }
    } catch (err) {
      console.error(err);
      setError("Erro ao conectar ao Spotify.");
      setConnectingSpotify(false);
    }
  };
  ...
  const handleDisconnectSpotify = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setError(data.error || "Erro ao desconectar Spotify.");
      }
    } catch (err) {
      console.error(err);
        ...
        {showConfirmCancel ? (
          ...
          <>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-zinc-950/60 border border-[#1DB954]/20 p-5 rounded-2xl space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="p-2 bg-[#1DB954]/20 border border-[#1DB954]/30 rounded-xl text-[#1DB954]">
                      <Music className="w-4 h-4" />
                    </div>
                    <div>
                      <h4 className="font-display font-black text-sm text-white uppercase tracking-tight">Integração Spotify</h4>
                      <p className="text-[10px] font-mono text-zinc-400">Controle de Mídia no Dashboard</p>
                    </div>
                  </div>

                  {spotifyConnected ? (
                    <span className="px-2 py-0.5 bg-[#1DB954]/20 border border-[#1DB954]/40 rounded-lg text-[9px] font-mono text-[#1DB954] font-bold uppercase flex items-center gap-1 shadow-[0_0_10px_rgba(29,185,84,0.2)]">
                ...
                <p className="text-xs text-zinc-400 leading-relaxed">
                  Conecte sua conta do Spotify via OAuth 2.0 para habilitar o Player de Mídia iOS/Apple Music diretamente no seu Dashboard do EA FC 26.
                </p>
                  ...
                  {spotifyConnected ? (
                    ...
                    >
                      {disconnectingSpotify ? (
                        ...
                        "Desconectar minha conta do Spotify"
                      )}
                    </button>
                  ) : (
                    <button
                      ...
                      className="w-full py-3 bg-[#1DB954] hover:bg-[#1ed760] text-black font-display font-black uppercase tracking-wider rounded-xl text-xs transition-all shadow-[0_0_15px_rgba(29,185,84,0.3)] hover:shadow-[0_0_22px_rgba(29,185,84,0.5)] cursor-pointer flex items-center justify-center gap-2 disabled:opacity-50"
                    >
                      {connectingSpotify ? (
                        <>
                        ...
                        <>
                          ...
                          Conectar minha conta do Spotify
                        </>
                      )}
                    </button>
                  )}
