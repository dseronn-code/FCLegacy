...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ adminUserId, onClose }) => {
  ...
  const toggleUserPro = async (userId: string, currentProState: boolean) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setUsersList(prev => prev.map(u => (u.docId === userId || u.userId === userId || u.email === userId) ? { 
          ...u, 
  ...
  const handleAddPlan = async (userId: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/users/${encodeURIComponent(userId)}/add-plan`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-user-id": adminUserId
  ...
  const handleRemovePlan = async (userId: string) => {
    ...
    setSubmittingPlan(true);
    try {
      const res = await fetch(`/api/admin/users/${encodeURIComponent(userId)}/remove-plan`, {
        method: "POST",
        headers: {
          "x-user-id": adminUserId
        }
  ...
  const resetUserAi = async (userId: string) => {
    try {
      const res = await fetch(`/api/admin/users/${encodeURIComponent(userId)}/reset-ai`, {
        method: "POST",
        headers: { "x-user-id": adminUserId }
      });
      const data = await res.json();
      if (data.success) {
        setUsersList(prev => prev.map(u => (u.docId === userId || u.userId === userId || u.email === userId) ? { ...u, aiGenerationsCount: 0 } : u));
  ...
  const banUser = async (userId: string) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setUsersList(prev => prev.map(u => (u.docId === userId || u.userId === userId || u.email === userId) ? { ...u, isBlocked: data.isBlocked } : u));
        fetchMetrics();
      }
    } catch (err) {
      console.error(err);
  ...
  const toggleUserAdmin = async (userId: string) => {
    try {
      const res = await fetch(`/api/admin/users/${encodeURIComponent(userId)}/toggle-admin`, {
        method: "POST",
        headers: { "x-user-id": adminUserId }
      });
      const data = await res.json();
      if (data.success) {
        setUsersList(prev => prev.map(u => (u.docId === userId || u.userId === userId || u.email === userId) ? { ...u, isAdmin: data.isAdmin } : u));
        fetchMetrics();
      }
    } catch (err) {
      console.error(err);
                          ...
                          <tr className={`border-b border-white/5 hover:bg-white/[0.01] transition-colors ${isExpanded ? "bg-white/[0.02]" : ""}`}>
                            ...
                            <td className="py-4 px-3 text-right">
                              <div className="flex items-center justify-end gap-1.5">
                                ...
                                <button
                                  onClick={() => toggleUserPro(user.userId || user.docId || user.email, !!user.isPro)}
                                ...
                                {/* Reset IA */}
                                <button
                                  onClick={() => resetUserAi(user.userId || user.docId || user.email)}
                                ...
                                <button
                                  onClick={() => toggleUserAdmin(user.userId || user.docId || user.email)}
                                  className={`p-1.5 rounded-lg border transition-all cursor-pointer ${
                                    user.isAdmin
                                ...
                                {/* Ban / Unban */}
                                <button
                                  onClick={() => banUser(user.userId || user.docId || user.email)}
                                  className={`p-1.5 rounded-lg border transition-all cursor-pointer ${
                                    user.isBlocked
                                ...
                                <div className="space-y-4 font-mono text-xs text-left animate-fadeIn">
                                  ...
                                  <div className="bg-zinc-950 p-4 border border-white/5 rounded-2xl space-y-3">
                                    ...
                                    <div className="flex flex-wrap gap-4 items-end pt-1">
                                      ...
                                      <button
                                        onClick={() => handleAddPlan(user.docId || user.userId || user.email)}
                                      ...
                                      {user.isPro && (
                                        <button
                                          onClick={() => handleRemovePlan(user.docId || user.userId || user.email)}
                                          disabled={submittingPlan}
