...
export const InteractiveQuestionnaire: React.FC<InteractiveQuestionnaireProps> = ({
  ...
  const handleRandomizeAll = async (e?: React.MouseEvent) => {
    e?.preventDefault();
    e?.stopPropagation();
    ...
    // Fetch AI-powered randomized draft from the backend first
    try {
    ...
    // If fetch failed or had no data, use our clean, original local names (NOT famous soccer players)
    if (!chosenName) {
            ...
            <form onSubmit={handleAIGenerateAndLoad} className="space-y-4">
              ...
              <div className="pt-4 flex flex-col sm:flex-row gap-4">
                <button
                  ...
                  className="px-5 py-3.5 bg-[#111113] hover:bg-zinc-800 border border-white/5 text-zinc-300 rounded-xl text-xs transition-all flex items-center justify-center gap-1.5 font-mono uppercase font-bold cursor-pointer flex-1 disabled:opacity-50"
                  title="Sortear um atleta 100% aleatório preenchendo todos os blocos com presets únicos"
                >
                  <RefreshCw className={`w-3.5 h-3.5 text-brand-green ${randomizing ? 'animate-spin' : 'animate-pulse'}`} />
                  {randomizing ? "Sorteando..." : "Sortear Preset"}
                </button>
