...
import { Trophy, Activity, Award, Loader2, User, Sparkles } from "lucide-react";
      ...
      (querySnapshot) => {
        ...
        querySnapshot.forEach((docSnap) => {
          ...
          leaderboardList.push({
            ...
            isPro: career.isPro || false,
            isBoosted: career.isBoosted || false
          });
        });
                ...
                {items.map((player, index) => {
                  ...
                  return (
                    <motion.tr
                      ...
                      className={`border-b cursor-pointer transition-all relative overflow-hidden group ${
                        player.isBoosted 
                          ? "border-brand-green/40 bg-brand-green/10 hover:bg-brand-green/20 shadow-[0_0_15px_rgba(36,208,129,0.15)]" 
                          : "border-white/5 hover:bg-brand-green/5"
                      }`}
                    >
                      ...
                      <td className="py-3 px-3">
                        <div className="font-display font-black text-white group-hover:text-brand-green transition-colors leading-tight flex items-center gap-1.5 flex-wrap">
                          <span>{player.playerName}</span>
                          {player.isPro && (
                            <span className="inline-flex items-center gap-0.5 px-1 py-0.5 bg-gradient-to-r from-yellow-400 to-amber-500 rounded text-[7px] font-extrabold text-black uppercase tracking-widest shadow-[0_0_10px_rgba(234,179,8,0.4)]">
                              <Sparkles className="w-1.5 h-1.5 text-black fill-black animate-spin-slow" />
                              PRO
                            </span>
                          )}
                          {player.isBoosted && (
                            <span className="inline-flex items-center gap-0.5 px-1 py-0.5 bg-brand-green text-black rounded text-[7px] font-extrabold uppercase tracking-widest shadow-[0_0_8px_rgba(36,208,129,0.5)] animate-pulse">
                              BOOST
                            </span>
                          )}
                        </div>
