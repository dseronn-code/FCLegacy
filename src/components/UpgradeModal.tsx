...
export const UpgradeModal: React.FC<UpgradeModalProps> = ({
      ...
      fetch("/api/plans")
        ...
        .then(data => {
          if (Array.isArray(data)) {
            setCustomPlans(data.filter(p => p.active !== false));
          }
        })
        .catch(err => console.error("Erro ao carregar planos do catálogo:", err));
    }
  }, [isOpen]);
  ...
  const handleSimulateApprove = async () => {
    ...
    try {
      const res = await fetch("/api/payments/simulate-approve", {
        ...
        body: JSON.stringify({ subscriptionId: checkoutData.subscriptionId })
      });
      const data = await res.json();
      if (data.success) {
        setPaymentSuccess(true);
  ...
  const handleCreateCheckout = async (purchaseType: string, planId?: string) => {
    setLoading(true);
    try {
          ...
          {!checkoutData ? (
            ...
            >
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                ...
                {/* Custom Admin Created Plans */}
                {customPlans.length > 0 && customPlans.filter(cp => !["plan_teste", "plan_pro", "plan_extra_slot", "plan_boost"].includes(cp.id)).map(plan => (
                  <button
                    key={plan.id}
                    disabled={loading}
                    onClick={() => handleCreateCheckout(plan.type || "pro", plan.id)}
                    className="w-full text-left p-4 bg-gradient-to-r from-emerald-500/10 to-teal-600/10 border border-emerald-500/30 hover:border-emerald-400 hover:from-emerald-500/15 hover:to-teal-600/15 rounded-2xl transition-all flex items-center justify-between group cursor-pointer"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5">
                        <span className="p-1 bg-emerald-400 text-black rounded-lg">
                          <Sparkles className="w-3 h-3 fill-black" />
                        </span>
                        <span className="text-xs font-mono font-black text-emerald-400 uppercase tracking-wider">{plan.name}</span>
                        {plan.badge && (
                          <span className="px-1.5 py-0.5 bg-yellow-400/20 text-yellow-300 text-[8px] font-mono uppercase font-bold rounded">
                            {plan.badge}
                          </span>
                        )}
                      </div>
                      <p className="text-[10px] text-zinc-400">{plan.description || `Plano de ${plan.days} dias`}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-base font-display font-black text-emerald-400 font-mono leading-tight">
                        R$ {Number(plan.price).toFixed(2)}
                        <span className="text-[9px] text-zinc-400 font-normal">/{plan.days}d</span>
                      </p>
                    </div>
                  </button>
                ))}
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <button
                  disabled={paymentSuccess || simulating}
                  onClick={handleSimulateApprove}
                  className="w-full py-3 bg-brand-green hover:bg-brand-green-hover text-black font-display font-black uppercase tracking-wider rounded-xl text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-[0_0_20px_rgba(34,197,94,0.3)]"
                >
                  {simulating ? (
                    <>
