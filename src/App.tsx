...
export default function App() {
  ...
  const saveCareerToCloud = async (updatedCareer: PlayerCareer) => {
    ...
    try {
      ...
      setCareer(updatedCareer);
      localStorage.setItem(`active_career_id_${currentUser.uid}`, updatedCareer.id);
      setCareersList(prev => {
        const exists = prev.some(c => c.id === updatedCareer.id);
        if (exists) {
          return prev.map(c => c.id === updatedCareer.id ? updatedCareer : c);
        } else {
          return [updatedCareer, ...prev];
        }
      });
    } catch (err) {
              ...
              <form onSubmit={handleVerifyAndRegister} className="space-y-4">
                ...
                {verificationDebugCode && (
                  <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 text-yellow-300 text-xs rounded-xl leading-normal text-center font-mono">
                    ...
                    <button 
                      type="button" 
                      onClick={() => setVerificationCodeInput(verificationDebugCode)}
                ...
                <div>
                  <label className="block text-[9px] font-mono text-zinc-400 uppercase tracking-widest mb-1.5 font-bold text-center">Código de 6 Dígitos</label>
                  <input
                    type="text"
                    maxLength={6}
                    ...
                    className="w-full text-center bg-black/40 border border-white/5 rounded-xl px-4 py-3.5 text-lg font-bold text-white tracking-widest focus:outline-none focus:ring-1 focus:ring-brand-green focus:border-brand-green transition-all font-mono"
                  />
                </div>
                ...
                {careersList.length > 0 && (
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full py-3 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 hover:text-white rounded-xl text-xs transition-all font-mono font-bold uppercase tracking-wider border border-white/5 hover:border-brand-green/30 cursor-pointer"
                    >
