    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        if (wasGuest) {
          ...
          const savedCustomSession = localStorage.getItem("custom_user_session");
          if (savedCustomSession) {
            try {
              const customUser = JSON.parse(savedCustomSession);
              setCareerLoading(true);
              localStorage.removeItem("is_guest_session");
              ...
              try {
                const q = query(collection(db, "careers"), where("userId", "==", customUser.uid));
                const querySnapshot = await getDocs(q);
                const list: PlayerCareer[] = [];
                querySnapshot.forEach((docSnap) => {
                  list.push(docSnap.data() as PlayerCareer);
  ...
  const handleAuth = async (e: FormEvent) => {
    ...
    if (authMode === "login") {
      ...
      try {
        ...
        console.warn("[AUTH CLIENT WARN] Tentando login via backend fallback...", err);
        try {
          ...
          const data = await res.json();
          if (res.ok && data.user) {
            localStorage.setItem("custom_user_session", JSON.stringify(data.user));
            setCurrentUser(data.user);
            setAuthError(null);
            
            // Query user's careers
            try {
              const q = query(collection(db, "careers"), where("userId", "==", data.user.uid));
              const querySnapshot = await getDocs(q);
              const list: PlayerCareer[] = [];
              querySnapshot.forEach((docSnap) => {
                list.push(docSnap.data() as PlayerCareer);
              ...
              console.warn("Erro ao buscar carreiras do usuário após login fallback:", careerErr);
            }
          } else {
            throw new Error(data.error || "E-mail ou senha incorretos.");
          }
        } catch (backendErr: any) {
          setAuthError(backendErr.message || "E-mail ou senha incorretos.");
        }
      } finally {
        setVerificationLoading(false);
      }
                ...
                {forgotSuccessMessage && (
                  <div className="p-3 bg-green-950/40 border border-green-500/20 rounded-xl text-green-300 text-[11px] flex flex-col gap-1.5 animate-fadeIn">
                    ...
                    {forgotDebugInfo?.code && (
                      <div className="mt-1 pt-1.5 border-t border-green-500/20 flex items-center justify-between">
                        <span className="text-[10px] font-mono text-yellow-400 font-bold">💡 Modo de Teste:</span>
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="text-[10px] font-mono text-black font-bold bg-brand-green hover:bg-[#d9ff33] px-2 py-1 rounded transition-colors cursor-pointer"
                        >
                          Preencher ({forgotDebugInfo.code}) & Verificar
                        </button>
                      </div>
                    )}
                  </div>
